...
const fetchCoSponsoredLegislation = async (
    ...
    if (!response.ok) {
        ...
        console.error(
            `Failed to fetch co-sponsored legislation: ${response.status} ${response.statusText} ${errorText}`
        );
        throw new Error(`Failed to fetch co-sponsored legislation: ${response.status} ${response.statusText}`);
    }
...
export default async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    try {
        ...
        const data = await fetchCoSponsoredLegislation(
            personId as string,
